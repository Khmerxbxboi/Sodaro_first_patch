...
$$('#businessTypes button').forEach(b=>b.addEventListener('click',()=>{$$('#businessTypes button').forEach(x=>x.classList.remove('active'));b.classList.add('active');business=b.dataset.business;updateDashboard()}));
const scenes={crime:{title:'Safety alert near Lake Merritt',body:'Sodaro suggests adjusting closing procedures, sharing the alert with staff, and offering pickup or delivery options.'},construction:{title:'Construction is reducing foot traffic',body:'The system connects the street closure to lower visits and recommends a timed promotion plus clearer wayfinding.'},event:{title:'Community event may increase visitors',body:'Sodaro recommends extending hours, preparing inventory, and posting an event-specific offer.'},code:{title:'Outdoor display rule changed',body:'A location-based alert explains the update in plain language and links it to the business owner’s next task.'}};
